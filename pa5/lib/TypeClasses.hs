...
dOrdXY ordA ordB = MkOrd compareXY
  where
    compareXY (a1, b1) (a2, b2) =
      case compare ordA a1 a2 of
        EQ -> compare ordB b1 b2
        r -> r

-- MkOrd compareXY
--   where
--     compareXY :: (a, b) -> (a, b) -> Ordering
--     compareXY (a1, b1) (a2, b2) = EQ
...
dOrdYX ordA ordB = MkOrd compareYX
  where
    compareYX (a1, b1) (a2, b2) =
      case compare ordB b1 b2 of
        EQ -> compare ordA a1 a2
        r -> r
...
exSortIntDesc :: [Int]
exSortIntDesc = sort dOrdDesc exIntList
  where
    dOrdDesc :: Ord Int
    dOrdDesc = MkOrd (\x y -> Prelude.compare y x)
...
isSorted _ [] = True
isSorted _ [_] = True
isSorted dOrd (x : x' : xs) =
  case compare dOrd x x' of
    GT -> False
    _ -> isSorted dOrd (x' : xs)

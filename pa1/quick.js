...
const partition = (g, ls) => {
  return List([filter(g, ls), filter((el) => !g(el), ls)]);
};
/* TASK 5 (15pts):
...
const quicksort = function (ls) {
  ...
  const partitions = partition((el) => el <= pivot, ls.shift());
  const p1 = partitions.first()
  const p2 = partitions.last()
  return quicksort(p1).concat(pivot).concat(quicksort(p2));
};

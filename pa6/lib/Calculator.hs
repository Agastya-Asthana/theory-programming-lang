...
interp :: Expr -> Int
interp (Lit i) = i
interp (Op op expr1 expr2) = interpBinOp op (interp expr1) (interp expr2)
...
peephole opt (Lit l) = opt (Lit l)
peephole opt (Op o (Lit l) e) = opt (Op o (peephole opt (Lit l)) (peephole opt e))
peephole opt (Op o e (Lit l)) = opt (Op o (peephole opt e) (peephole opt (Lit l)))
peephole opt (Op o exp1 exp2) = opt (Op o (peephole opt exp1) (peephole opt exp2))
...
step (IPush i) s = Just (i : s)
step (IOp op) (i1 : i2 : stack) = Just newStack
  where
    newStack = val : stack
    val = interp (Op op (Lit i2) (Lit i1))
step (IOp _op) _stack = Nothing
...
run [] s = Just s
run instrs st = foldM f st instrs
  where
    f s inst = step inst s
...
compile :: Expr -> [Instr]
compile (Lit l) = [IPush l]
compile (Op o exp1 exp2) = comp1 ++ comp2 ++ z
  where
    comp1 = compile exp1
    comp2 = compile exp2
    z = [IOp o]
